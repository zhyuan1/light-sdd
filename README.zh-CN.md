...
轻量、可编排、可插拔的 Spec-Driven Development 工作流，支持多种 AI 编程助手（Claude Code、CodeBuddy 等）。
...
执行 `./install.sh` 后，以下文件会被放置到你 CLI 的配置目录下（如 `~/.codebuddy/` 或 `~/.claude/`）：

```
~/.{config_dir}/
  skills/
    sdd-templates/          # SDD 共享配置与模板
      delegates.yaml        # action -> 委派 skill 注册表（primary、fallback、profiles）
      delegation-protocol.md  # skill 解析算法
      schema.yaml           # 7 种产物类型的内容约束
      brainstorm.md         # 产物模板
      proposal.md
      spec.md
      design.md
...
每个 SDD skill 启动时都会搜索 `sdd-templates/` 目录，以定位 `delegates.yaml`、`delegation-protocol.md` 和产物模板。搜索顺序：优先项目级路径，再查用户级路径，覆盖所有已知 CLI 配置目录（`codebuddy`、`claude`、`claude-internal`）。
...
| 框架 | 用于 | 是否必需 |
|------|------|---------|
| [OpenSpec](https://github.com/fission-ai/openspec) | propose, ff, verify, ship | 推荐 |
| [Superpowers](https://github.com/obra/superpowers) | brainstorm, plan, code, review-code, verify, ship | 推荐 |
| [ECC](https://github.com/badass-courses/everything-claude-code) | brainstorm（备选） | 可选 |

按需安装，不必全装。SDD 在运行时自动检测各框架是否可用 -- 未找到时会降级到备选 skill 或手动模式，不会报错中断。

> **多 CLI 支持**：安装器会自动检测你的 CLI 工具。它按顺序检查 `~/.codebuddy`、`~/.claude` 等目录，安装到第一个找到的目录。使用 `--target` 可手动指定。下面的示例中，将 `~/<config_dir>/skills/` 替换为你 CLI 的配置路径（如 `~/.codebuddy/skills/` 或 `~/.claude/skills/`）。
...
# 初始化 skill 文件（必须执行 -- 仅安装 CLI 不够）
openspec init --tools claude

# 验证安装 -- 以下 skill 文件应已存在
# （路径取决于你的 CLI：~/.claude/skills/、~/.codebuddy/skills/ 等）
...
ECC 是可选依赖，仅在未安装 Superpowers 时作为备选使用。

```bash
git clone https://github.com/badass-courses/everything-claude-code
cd everything-claude-code
./install.sh          # 将 skills、agents、commands 安装到 CLI 配置目录

# 验证安装（使用你 CLI 的 skills 目录）
ls ~/<config_dir>/skills/ | grep -E "^think$|^check$|^plan$|^hunt$"
```

#### gstack

gstack 仅在切换到 `gstack` profile（`/sdd-use gstack`）时才需要安装。

```bash
# 将 ~/<config_dir> 替换为你 CLI 的配置目录（如 ~/.codebuddy、~/.claude）
git clone https://github.com/garrytan/gstack ~/<config_dir>/skills/gstack

# 验证安装 -- 以下 skill 文件应已存在
ls ~/<config_dir>/skills/gstack | grep -E "office-hours|review|ship|qa"
```

> **注意**：gstack 需要 [Bun](https://bun.sh)（v1.0+）。安装命令：`curl -fsSL https://bun.sh/install | bash`
...
每个委派型 action 在 Pre-check 阶段包含 **Delegation availability check** 步骤。它会在所有已知 CLI 的标准 skill 路径（`~/.{config_dir}/skills/`、`~/.{config_dir}-internal/skills/`、`.{config_dir}/skills/`、项目配置路径）中搜索目标 skill。如果未找到：
